...
describe('exportStorylinePackages', () => {
  it('triggers export, downloads, repackages each leaf, writes a manifest', async () => {
    ...
    const summary = await exportStorylinePackages(store, onEvent, { exportOne, fetchZip, pacing: { baseMs: 0, jitterMs: 0 } });
  ...
  it('skips a course that already has a manifest (resume)', async () => {
    ...
    const summary = await exportStorylinePackages(store, onEvent, { exportOne, fetchZip: vi.fn() });
    expect(summary).toMatchObject({ packaged: 0, skipped: 1 });
    expect(exportOne).not.toHaveBeenCalled();
  });

  it('records a per-course failure without throwing', async () => {
    const { store } = makeStorage();
    const { onEvent } = sink();
    const exportOne = vi.fn(async () => ({ ok: false as const, error: 'ws.eu refused' }));
    const summary = await exportStorylinePackages(store, onEvent, { exportOne, fetchZip: vi.fn() });
    expect(summary).toMatchObject({ packaged: 0, failed: 1 });
    expect(summary.errors[0]).toEqual({ courseId: 'C1', error: 'ws.eu refused' });
  });
});
