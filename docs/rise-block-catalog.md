...
| `image/text aside` | Image beside text | `items[0]{media.image{key,crushedKey,dimensions,isSkipCrush,originalUrl,sourcedFrom,useCrushedKey}, caption, paragraph}`; `settings{imageSize, imagePosition, opacity, opacityColor, mightyMods[mighty-rise-block-mod-image-and-text]}` — the VAS lesson-intro donor (style profile) | image key (+crushed) | — | 2026-09-14 (VAS 1.1–1.3, 39 blocks) | documented |
| `image/text overlay` | Full-width banner with caption over a picture | `items[0]{media.image, caption (HTML), paragraph:''}`; `settings{opacity:'0.1', opacityColor, cardMode:'WHITE', customPadding*, entranceAnimation}`; `background:{}`, `data:{}` — the VAS section banners (Kopsavilkums / Uzdevums) | image key | — | 2026-09-14 (VAS, 26 blocks) | documented |
| `image/banner` | Full-width picture banner | `items[0]{media.image, caption}`; `mightyMods[mighty-rise-block-mod-background-gradient]` | image key | — | 2026-09-14 (VAS) | seen |
| `quote/d` | Quote card with avatar icon + background | `items[0]{name (HTML attribution), avatar.media.image, paragraph (HTML), background.media.image (library default `assets/rise/assets/block-defaults/quote_background.jpg`)}`; `settings{avatar, avatarSize, quoteWidth, quotePadding, quotesInline}` — the VAS "Tēmas padziļināšanai" deep-dive donor | avatar key (+crushed); library background | — | 2026-09-14 (VAS, 18 blocks) | documented |
| `interactive/tabs` + `settings.mightyBlockConfig` | **Mighty "Interactive HTML"** custom-code block surfacing as tabs | `items[]` = Mighty's two placeholder tabs ("Mighty placeholder" / "Learn more"); the content is `settings.mightyBlockConfig{id (uuid, per block), name:'Interactive HTML', tagName:'mighty-interactive-html', version:0, blockOptions{layout,autocomplete}, data{inputMode:'direct', directHtml, directCss, injectCss, injectJavascript, showBorder, autocomplete, interactiveWidth}}` — the VAS YouTube card (4 HTML generations; the newest is the profile's video donor, video id templated) | none (YouTube embed URL inside `directHtml`) | — | 2026-09-14 (VAS 1.1: 13, 1.3: 5 placeholders) | documented |
| `multimedia/attachment` | Downloadable file | `items[0].media.attachment{key,size,type:'attachment',filename,mimeType,originalUrl}`; optional `items[1]` = icon image attachment referenced by `mightyMods[mighty-rise-block-mod-attachment-icon].image` (= items[1].id); `mightyMods[mighty-rise-block-mod-attachment-appearance]` | file key + icon key | — | 2026-09-14 (VAS) | documented |
