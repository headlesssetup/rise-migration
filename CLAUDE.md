...
- **Loud failure is scoped to media keys and cross-refs.** Run a generic recursive scan over the whole course document for media-key-shaped values (S3 keys, `rise/courses/{id}/…` paths, `articulateusercontent.com` URLs) and cross-refs. If a media key or cross-ref is found that the remap plan does not cover, ABORT that course with the exact path + raw snippet. Do NOT abort merely because a block `family/variant` is unseen — copy it faithfully.
